...
[![CI](https://github.com/TokensAPI/tokens_DshProgressiveTools_code/actions/workflows/checks.yml/badge.svg)](https://github.com/TokensAPI/tokens_DshProgressiveTools_code/actions/workflows/checks.yml)
...
- `src/`: runtime implementation; `tests/`: regression tests.
...
- `.github/workflows/checks.yml`: multi-version Node checks on branches and pull requests.
- `.github/workflows/publish-npm.yml`: tag-gated release to the private registry.
...
Keep generated output out of Git. Keep both README languages, legal notices,
and the small `CLAUDE.md` pointer to the authoritative `AGENTS.md`.
...
executes real business tools. See [evaluation](./evaluation.md) for methodology.
...
See [configuration](./configuration.md) for every option, the
...
[discovery boundaries](./architecture.md#discovery-boundaries) map Skills,
